...
class BoardRenderer{
    ...
    public function show(){
        ...
        for($y=0;$y<$this->boardSet->height;$y++) {
            for($x=0;$x<$this->boardSet->width; $x++){
                echo $this->getTileDiv($this->boardSet->tile_cover[$x][$y], $this->boardSet->tiles[$x][$y]);
            }
    ...
    private static function getTileDiv($is_covered, $block){
        $image = "tile.png";
        if(!$is_covered){
        ...
        return '<div class="tile" style=\'background-image: url("Assets/'.$image.'")\'></div>';
    }
}